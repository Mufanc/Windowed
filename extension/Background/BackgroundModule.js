...
let is_valid_window = (window) => {
    return window.incognito === false && window.type === 'normal' && window.state !== 'minimized'
}
...
onMessage('update_windowed_button', async (message, sender) => {
    let tabs = message.id ? [await browser.tabs.get(message.id)] : await browser.tabs.query(message.query)
    for (let tab of tabs) {
...
onMessage('please_make_me_a_popup', async (message, sender) => {
    ...
    let { left: screenLeft, top: screenTop, type: windowType } = await browser.windows.get(sender.tab.windowId)
...
let icon_theme_color = async (tab) => {
    ...
    return window.matchMedia('(prefers-color-scheme: dark)').matches ? 'rgba(255,255,255,0.8)' : '#5f6368'
}
...
let update_button_on_tab = async (tab) => {
    ...
    if (tab.status === 'complete' && has_contentscript_active === false) {
        await apply_browser_action(tab.id, {
            ...
            title: 'This page needs to be reloaded for Windowed to activate. Click here to reload.',
        })
        return
    }
